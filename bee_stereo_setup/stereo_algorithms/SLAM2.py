...
def detect_features(frame, max_features=150, min_dist=15):
    """
    Detects well-spread feature points in a frame using AKAZE.
    Draws green dots on the detected points in the input frame.
    
    Args:
        frame: BGR image
        max_features: max number of features to retain
        min_dist: minimum distance (pixels) between features
    ...
    # Filter features to enforce minimum distance
    selected_kps = []
    selected_desc = []
    ...
    for i, kp in enumerate(keypoints):
        u, v = kp.pt
        too_close = False
        for sel_kp in selected_kps:
            if np.linalg.norm(np.array([u, v]) - np.array(sel_kp.pt)) < min_dist:
                too_close = True
                break
        if not too_close:
            selected_kps.append(kp)
            selected_desc.append(descriptors[i])
        if len(selected_kps) >= max_features:
            break

    selected_desc = np.array(selected_desc, dtype=descriptors.dtype)
    ...
    pts_2d = np.array([kp.pt for kp in selected_kps], dtype=np.float32)

    return pts_2d, selected_desc
...
class CreStereoNode(Node):
    ...
    def stereo_callback(self, left_msg, right_msg):
        ...
        if not self.keyframe_initialized:
            ...
            # 2️⃣ Boundary check (remove points near image border)
            h, w = gray.shape
            inside_mask = (
            ...
            # # 3️⃣ Validation only every N frames
            # if self.frame_count % self.track_interval == 0:
            #     matched_2D, matched_3D = validate_and_fix_tracked_pts(
            #         gray,
            #         good_new,
            #         keyframe=map_db["keyframe_1"],
            #         radius=10
            #     )
            #     self.get_logger().info(
            #         f"Frame {self.frame_count}: revalidated {matched_2D.shape[0]} correspondences."
            #     )
            # else:
            #     # Use tracked pts directly, assume index alignment with keyframe landmarks
            #     matched_2D = good_new
            #     matched_3D = map_db["keyframe_1"]["pts_3d"][:len(good_new)]
            # Use tracked pts directly, assume index alignment with keyframe landmarks
            matched_2D = good_new
            matched_3D = map_db["keyframe_1"]["pts_3d"][:len(good_new)]
            # 4️⃣ Update optical flow state
            self.prev_gray = gray
            self.prev_pts = good_new.reshape(-1, 1, 2)

            # 5️⃣ Draw tracked points for debug
            for pt in good_new:
            ...
            if matched_2D.shape[0] >= 4:  # PnP requires at least 4 points, 6+ is safer
                ...
                if success:
                    ...
                    self.get_logger().info(
                        ...
                        # f"inliers={inliers}points"
                    )
                else:
                    self.get_logger().warn("PnP failed to find a solution.")
