...
class DepthFilter:
    def __init__(self, alpha=0.8):
        ...
        alpha: smoothing factor [0,1]
               - closer to 1 → smoother (slower to change)
               - closer to 0 → more responsive
        """
        self.alpha = alpha
        self.prev_depth = None

    def filter(self, depth_map):
        """
        depth_map: np.ndarray (H, W), new frame of depth values
        returns: smoothed depth map
        """
        if self.prev_depth is None:
            self.prev_depth = depth_map.astype(np.float32)
            return self.prev_depth

        # EMA: prev + alpha * (current - prev)
        self.prev_depth = self.prev_depth + self.alpha * (depth_map - self.prev_depth)
        return self.prev_depth
# ===========================================================================
# ============================== Main class ==============================
class CreStereoNode(Node):
    def __init__(self):
        ...
        self.kf_pub = self.create_publisher(Float64MultiArray, "keyframe_bundle", 10)
        ...
        self.world_T_cam = np.eye(4, dtype=np.float32)  # identity for first frame
        self.last_keyframe_T = np.eye(4, dtype=np.float32)
        ...
        self.min_rotation = 0.2     # radians

        # filter
        # self.depth_filter = DepthFilter(alpha=0.8)
        # Global state
    ...
    def publish_keyframe(self, keypoints2d: np.ndarray, landmarks3d: np.ndarray, pose6: list):
        """
        keypoints2d: (N x 2) np.float32 or np.float64
        landmarks3d: (M x 3) np.float32 or np.float64
        pose6: [tx, ty, tz, roll, pitch, yaw]  length 6
        """
        # ensure numpy arrays
        k2 = np.asarray(keypoints2d).reshape(-1, 2) if keypoints2d is not None else np.empty((0,2))
        l3 = np.asarray(landmarks3d).reshape(-1, 3) if landmarks3d is not None else np.empty((0,3))
        n2 = int(k2.shape[0])
        n3 = int(l3.shape[0])

        data_list = []
        # header: sizes (stored as floats)
        data_list.extend([float(n2), float(n3), 6.0])

        # flatten 2D as u0, v0, u1, v1, ...
        if n2 > 0:
            data_list.extend(k2.reshape(-1).tolist())

        # flatten 3D as X0, Y0, Z0, X1, Y1, Z1, ...
        if n3 > 0:
            data_list.extend(l3.reshape(-1).tolist())

        # pose (6 floats) - pad/truncate to 6
        pose6 = list(pose6) if pose6 is not None else [0.0]*6
        if len(pose6) < 6:
            pose6 = pose6 + [0.0]*(6 - len(pose6))
        data_list.extend([float(x) for x in pose6[:6]])
    ...
    def stereo_callback(self, left_msg, right_msg):
        ...
        is_first_frame = self.tracker.active_points is None
        ...
        if len(pt_3d_prev) > 0 and len(pt_2d_surv) > 0:
            if len(valid_2d) >= 10:   # at least 4 for PnP, 10+ is safer
                # --- Step 1: RANSAC + EPNP (fast, robust to outliers) ---
                ...
                if success:
                    # --- Step 2: Iterative refinement (better rotation) ---
                    ...
                    # --- Step 5: Extract translation & orientation ---
                    tx, ty, tz = self.world_T_cam[:3, 3]

                    
                    r = Rot.from_matrix(self.world_T_cam[:3, :3])
                    roll, pitch, yaw = r.as_euler('zxy', degrees=False)
        ...
        # =================================================================
    # ========= Sending keypoints, landmarks and keyframe pos =================
    # sending [valid_2d] , [valid_3d] , [tx, ty, tz , roll, pitch, yaw] here
        # Initialize last keyframe pose
        if not hasattr(self, "last_keyframe_T"):
            self.last_keyframe_T = self.world_T_cam.copy()
            first_frame = True
        else:
            first_frame = False

        # Compute relative motion since last keyframe
        delta_T = np.linalg.inv(self.last_keyframe_T) @ self.world_T_cam
        delta_trans = np.linalg.norm(delta_T[:3, 3])
        # delta_rot = cv2.norm(delta_T[:3, :3] - np.eye(3), cv2.NORM_FRO)  # rough rotation norm
        delta_rot = np.linalg.norm(delta_T[:3, :3] - np.eye(3), 'fro')

        # Decide keyframe
        if delta_trans >= self.min_translation or delta_rot >= self.min_rotation or first_frame:
            # Update last keyframe pose
            self.last_keyframe_T = self.world_T_cam.copy()

            # Extract translation & orientation
            pose6 = [tx, ty, tz, roll, pitch, yaw]

            # Publish
            self.publish_keyframe(valid_2d, valid_3d, pose6)    #store sparse 3D landmarks
            #Storing dense downsample pcd as landmarks
            # self.publish_keyframe(valid_2d, points[np.random.choice(points.shape[0], int(0.3 * points.shape[0]), replace=False)], pose6) 
