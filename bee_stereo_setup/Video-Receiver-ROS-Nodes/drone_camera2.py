...
# Saving params
# SAVE_DIR = "bee/perception/stereo_right"
...
class UdpCamNode(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(Image, '/drone_cam2', 10)
    ...
    def process_frame(self, data):
        ...
        if img is not None:
            ...
            # Display image
            cv2.imshow('ESP32 Stream 2', rotated_img)
            cv2.waitKey(1)
            # Handle key press
    ...
    def receive_udp(self):
        ...
        if frame_id != self.last_frame_id and self.last_frame_id in frame_buffers:
            ...
            self.get_logger().info(f"[Frame {frame_id}] SChunk size: {len(data)} bytes")
